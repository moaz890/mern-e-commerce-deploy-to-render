...
export const createOrder = createAsyncThunk('order/createOrder', async (orderData) => {
    const response = await axios.post(`${import.meta.env.ORIGIN_UR}/api/shop/order/create`, orderData);
    return response.data;
});

export const capturePayment = createAsyncThunk('order/captureOrder', async ({ paymentId, orderId, payerId }) => {
    const response = await axios.post(`${import.meta.env.ORIGIN_UR}/api/shop/order/capture`, { paymentId, orderId, payerId });
    return response.data;
});

export const getAllOrdersByUser = createAsyncThunk('order/getOrderByUser', async (userId) => {
    const response = await axios.get(`${import.meta.env.ORIGIN_UR}/api/shop/order/list/${userId}`);
    return response.data;
});

export const getOrdersDetails = createAsyncThunk('order/getOrderDetails', async (id) => {
    const response = await axios.get(`${import.meta.env.ORIGIN_UR}/api/shop/order/details/${id}`);
    return response.data;
});
